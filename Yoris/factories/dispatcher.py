...
class Dispatcher:
    ...
    def message_convert_to_context(self, message: aiogram.types.Message):
        if message.content_type == ContentType.TEXT:
            return contexts.MessageContext(message)
        if message.content_type == ContentType.AUDIO or message.content_type == ContentType.VIDEO or message.content_type == ContentType.DOCUMENT or message.content_type == ContentType.ANIMATION:

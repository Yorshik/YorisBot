...
class AppendContainsExactlyTrigger(TriggerBase):
    def __init__(self):
        self.author = None
        self.chat = None
        self.outcome = None
    ...
    async def execute(self, ctx: contexts.MessageContext):
        trigger = await database_manager.add_trigger(chat=self.chat, author=self.author, type="contains_exactly",
                                                          mode="dynamic", is_enabled=None, contains_exactly_income=self.income,
                                                          contains_exactly_outcome=self.outcome, name=self.name)
        await ctx.reply(_("Trigger was created by {author_name}\nTrigger id: {id}", ctx=ctx, author_name=self.author.name, id=trigger.id))


class TerminalAppendContainsExactlyTrigger(AppendContainsExactlyTrigger):
    ...
    async def matches(self, ctx: contexts.MessageContext) -> bool:
        ...
        try:
            ...
            return False
        if parsed.type not in ["contains_exactly", "contains-exactly"]:
            return False
...
class RemoveContainsExactlyTrigger(TriggerBase):
    def __init__(self):
        self.key = None
        self.chat = None
...
class TerminalRemoveContainsTrigger(RemoveContainsExactlyTrigger):
    factory_type = "trigger"
...
class ExecutorContainsExactlyTrigger(TriggerBase):
    ...
    async def matches(self, ctx: contexts.MessageContext) -> bool:
        self.chat = await database_manager.get_chat(ctx.chat.id)
        self.triggers = await database_manager.get_triggers(chat=self.chat, type="contains_exactly")
        text = ctx.text.strip().lower()
        for trigger in self.triggers:
            if trigger.contains_exactly_income.lower() in text:
    ...
    async def execute(self, ctx: contexts.MessageContext):
        await ctx.reply(self.trigger_to_execute.contains_exactly_outcome)
